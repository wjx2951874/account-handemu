...
A='7666692683432429819'; UID='717344312142287'; SEC='MS4wLjABAAAASUWIr_SDLMfbpb6mbeGrx_5fKU6NG7z1msxOWgiwYwYwU'
...
async def main():
 ...
 async with async_playwright() as p:
  ...
  await page.goto(f'https://www.douyin.com/note/{A}',wait_until='domcontentloaded',timeout=120000); await page.wait_for_timeout(10000)
  ...
  pat=re.compile(r'(展开\s*\d*\s*条?回复|查看\s*\d*\s*条?回复|更多回复|查看全部回复)'); stale=0; last=0; clicks=[]
  for step in range(650):
   hit=0; bs=page.locator('button'); n=min(await bs.count(),700)
   for i in range(n):
    q=bs.nth(i)
    try:
   ...
   try:
    await page.evaluate("""()=>{let a=[...document.querySelectorAll('*')].filter(e=>{let s=getComputedStyle(e),r=e.getBoundingClientRect();return r.height>180&&r.width>220&&r.bottom>0&&r.top<innerHeight&&/(auto|scroll)/.test(s.overflowY)&&e.scrollHeight>e.clientHeight+100}).sort((a,b)=>b.clientHeight*b.clientWidth-a.clientHeight*a.clientWidth);a.slice(0,10).forEach(e=>e.scrollBy(0,Math.max(700,e.clientHeight*.85)));window.scrollBy(0,900)}""")
   except: await page.mouse.wheel(0,1200)
   ...
   if step>120 and stale>=45:break
  await page.wait_for_timeout(4000); await page.screenshot(path=str(O/'抓取结束.png')); json.dump(clicks,open(O/'点击记录.json','w',encoding='utf8'),ensure_ascii=False)
  await b.close()
 D={}
 for x in P:
 ...
 for c in sorted(ai,key=lambda x:int(x.get('create_time') or 0)):
  ...
  for j,e in enumerate(entries(c),1):
   ...
   for u in urls(e):
    try:
     r=ses.get(u,timeout=35); r.raise_for_status(); ct=r.headers.get('content-type','').lower(); ext='.png' if 'png' in ct else '.webp' if 'webp' in ct else '.gif' if 'gif' in ct else '.jpg'
