...
def user_matches(d: dict[str, Any]) -> bool:
    values = {
        str(d.get("unique_id") or ""),
        str(d.get("short_id") or ""),
        str(d.get("douyin_id") or ""),
    }
...
def video_candidates(item: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for br in video.get("bit_rate") or []:
        ...
        if not urls:
            continue
        candidates.append({
            "source": "bit_rate.play_addr",
            "bit_rate": int(br.get("bit_rate") or 0),
            "quality_type": br.get("quality_type"),
            "gear_name": br.get("gear_name"),
            "width": int(br.get("play_addr", {}).get("width") or video.get("width") or 0) if isinstance(br.get("play_addr"), dict) else int(video.get("width") or 0),
            "height": int(br.get("play_addr", {}).get("height") or video.get("height") or 0) if isinstance(br.get("play_addr"), dict) else int(video.get("height") or 0),
            "data_size": int(br.get("play_addr", {}).get("data_size") or br.get("data_size") or 0) if isinstance(br.get("play_addr"), dict) else int(br.get("data_size") or 0),
            "urls": urls,
        })
    for key in ("play_addr", "play_addr_h264", "play_addr_265"):
        ...
        if urls:
            ...
            candidates.append({
                ...
                "quality_type": None,
                "gear_name": None,
                "width": int(addr.get("width") or video.get("width") or 0),
                "height": int(addr.get("height") or video.get("height") or 0),
                "data_size": int(addr.get("data_size") or 0),
                "urls": urls,
            })
    # Explicitly do not use download_addr because it is commonly watermarked.
    for c in candidates:
...
def normalize_aweme(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "aweme_id": aweme_id(item),
        "create_time": int(item.get("create_time") or 0),
        "create_time_text": fmt_time(item.get("create_time")),
        "desc": str(item.get("desc") or item.get("preview_title") or ""),
        "aweme_type": item.get("aweme_type"),
        "media_type": item.get("media_type"),
        "is_video": bool(video and candidates),
...
def main() -> None:
    ...
    ua = headers.get("user-agent") or "Mozilla/5.0 (X111; Linux x86_64) AppleWebKit/537.36 Chrome/150 Safari/537.36"
    diag: dict[str, Any] = {
        "target_id": TARGET_ID,
    ...
    with sync_playwright() as p:
        ...
        # Search the exact public account.
        ...
        try:
            ...
            body = ""
        diag["login_blocker"] = any(s in body for s in ("登录后可以", "扫码登录", "验证瀁登录", "密码登录"))

        # Scroll the search page a little so lazy-loaded user results arrive.
        for _ in range(15):
            page.mouse.wheel(0, 1200)
            page.wait_for_timeout(500)

        # Try to discover user link from visible DOM.
        ...
        # Deduplicate exact user candidates.
        ...
        if not sec_uid:
            for href in visible_user_urls:
                ...
                if m:
                    ...
                    break
        if not sec_uid:
            # Generic fallback: inspect all user links and their nearby text.
            try:
                links = page.locator("a[href*='/user/']")
        ...
        diag["selected_user"] = {
            "sec_uid": sec_uid,
            "uid": str(selected_user.get("uid") or ""),
            "unique_id": str(selected_user.get("unique_id") or ""),
            "short_id": str(selected_user.get("short_id") or ""),
            "nickname": str(selected_user.get("nickname") or ""),
        }
        dump(ROOT / "搜索用户候项.json", exact_users)
        ...
        page.wait_for_timeout(12000)
        page.screenshot(path=str(DIAG / "02_主页判始.png"), full_page=False)

        # Gather initial page embedded JSON as a fallback.
        try:
            html = page.content()
            (DIAG / "主页判始.html").write_text(html, encoding="utf-8")
            # The page often embeds aweme JSON in scripts; use a bounded regex search for IDs only.
            for m in re.finditer(r'"aweme_id'\s*:\s*'(\d{15,22})'", html):
                aid = m.group(1)
                # IDs discovered only from HTML are noted but not treated as full metadata.
        ...
        for loop in range(260):
            # Scroll the largest scrollable container plus the window.
            try:
                state = page.evaluate("""() => {
                  const els=[...document.querySelectorAll('*')].filter(e=>{
                  const s=getComputedStyle(e); return (s.overflowY==='auto'||s.overflowY==='scroll') && e.scrollHeight>e.clientHeight+120 && e.clientHeight>250;
                  });
                  els.sort((a,b)=>(b.scrollHeight*b.clientHeight)-(a.scrollHeight*a.clientHeight));
                  const e=els[0];
                  if(e){e.scrollTop=Math.min(e.scrollHeight,e.scrollTop+Math.max(1200,e.clientHeight*.95));e.dispatchEvent(new Event('scroll',{bubbles:true}));return {found:true,top:e.scrollTop,max:e.scrollHeight-e.clientHeight};}
                  window.scrollBy(0,1400); return {found:false,top:window.scrollY,max:document.documentElement.scrollHeight-innerHeight};
                }""")
            except Exception:
            ...
            if loop in (20, 60, 120, 200, 259):
                try:
                    page.screenshot(path=str(DIAG / f"主页滚动_M�S�!6